package com.example.servicecontrolleractivity;
...
public class ServiceControllerActivity extends Activity {
	...
	// startService(new Intent(ServiceControllerActivity.this,
	// MyService.class));
	// break;
	...
	private OnClickListener btnListener = new OnClickListener() {
		public void onClick(View v) {
			...
			case R.id.StopButton:// stopServiceでサービス終了
				stopService(new Intent(ServiceControllerActivity.this,
						MyService.class));
				break;
	...
	private ServiceConnection mConnection = new ServiceConnection() {
		public void onServiceConnected(ComponentName className, IBinder service) {
			...
			// サービスとの接続確立時に呼び出される
			Toast.makeText(ServiceControllerActivity.this,
					"Activity:onServiceConnected", Toast.LENGTH_SHORT).show();

			// サービスにはIBinder経由で#getService()してダイレクトにアクセス可能
			mBoundService = ((MyService.MyServiceLocalBinder) service).getService();

			// 必要であればmBoundServiceを使ってバインドしたサービスへの制御を行う
			mBoundService.runTimer();
		}
	...
	void doBindService() {
		...
		bindService(
				new Intent(ServiceControllerActivity.this, MyService.class),
